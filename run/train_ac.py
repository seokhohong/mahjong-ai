...
from core.learn.pure_policy_dataset import extract_indexed_state, _tile_index_from_str  # type: ignore
from core.learn_ac.ac_constants import chi_variant_index
from core.learn_ac.policy_utils import flat_index_for_action
from core.game import Tile, Suit, TileType
...
class ACDataset(Dataset):
    def __init__(self, npz_path: str, net: ACNetwork):
        ...
        actions = data['actions']  # object array of serialized action dicts
        ...
        # For flat policy, we only need old_log_probs
        ...
        for i in range(len(states)):
            sd = states[i].item() if isinstance(states[i], np.ndarray) else states[i]
            ad = actions[i].item() if isinstance(actions[i], np.ndarray) else actions[i]
            idx_state = extract_indexed_state(sd)
            h, c, d, g = net._extract_features_from_indexed(
                idx_state['hand_idx'],
                idx_state['disc_idx'],
                idx_state['called_sets_idx'],
                idx_state['game_state'],
            )
            ...
            fi = flat_index_for_action(sd, ad)
            flat_idx_list.append(int(fi))
        ...
        self.flat_idx = np.asarray(flat_idx_list, dtype=np.int64)
...
def train_ppo(
    ...
    if init_model:
        try:
            net.load_model(init_model, load_entire_module=False)
    ...
    if warm_up_acc and warm_up_acc > 0.0:
        ...
        while epoch < int(max(1, warm_up_max_epochs)) and not reached:
            ...
            # Default to train accuracy if no validation split
            epoch_acc = (correct / max(1, total_examples))
            ...
            if dl_val is not None:
                ...
                print(f"\nWarmUp {epoch+1} [train] - acc: {epoch_acc:.4f}")
    ...
    for epoch in range(epochs):
        ...
        if dl_val is not None:
            if v_avg < best_loss - 1e-6:
                ...
                torch.save(model.state_dict(), os.path.join(out_dir, f'ac_ppo_best_{timestamp}.pt'))
            else:
                epochs_no_improve += 1
