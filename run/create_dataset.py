...
from core.learn_ac import RecordingACPlayer, RecordingHeuristicACPlayer
...
def build_ac_dataset(
    ...
    if not use_heuristic:
        net = ACNetwork(hidden_size=hidden_size, embedding_dim=embedding_dim, max_turns=max_turns, temperature=temperature)
    ...
    for gi in tqdm(range(max(1, int(games)))):
        game = SimpleJong(players, tile_copies=constants.TILE_COPIES_DEFAULT)
        game.play_round()

        winners = list(game.get_winners()) if hasattr(game, 'get_winners') else []
        loser = game.get_loser() if hasattr(game, 'get_loser') else None
        # Let players update their terminal reward internally for consistency
        for p in players:
        ...
        for pid, p in enumerate(players):
            ...
            if T > 0:
                ...
                if pid in winners:
                    terminal = 1.0
                elif loser is not None and int(loser) == int(pid):
                    terminal = -1.0
            ...
            for t in range(T):
                ...
                all_states.append(serialize_state(gs))
...
def main():
    ...
    ap.add_argument('--embedding_dim', type=int, default=4)
