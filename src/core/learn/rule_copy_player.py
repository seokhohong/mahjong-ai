...
from .rule_copy_network import RuleCopyNetwork
...
class RuleCopyPlayer(Player):
    """Player using RuleCopyNetwork (same features/arch as PurePolicyNetwork).

    Selects the highest-probability legal action under the flattened action space.
    """

    def __init__(self, player_id: int, network: RuleCopyNetwork):
    ...
    def _select_best_legal(self, gs: GamePerspective, legal_moves: List[Any]) -> Optional[Any]:
        ...
        # Highest-probability among legal
        _, best_move = max(action_pairs, key=lambda p: float(probs[p[0]]))
        return best_move
