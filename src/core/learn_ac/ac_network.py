...
class ACNetwork:
	"""
	Actor-Critic network for SimpleJong with shared trunk and multiple heads:
	...
	def __init__(self, hidden_size: int = 128, embedding_dim: int = 4, max_turns: int = MAX_TURNS, temperature: float = 1.0):
		...
		self._max_called_tiles_per_player = 9
		...
		class _ACModule(nn.Module):
			def __init__(self, outer: 'ACNetwork') -> None:
				...
				# Heads: single flat policy (18 discards + 3 chi variants + 4 mains = 25)
				self.head_policy = nn.Linear(outer.hidden_size // 2, 25)
		...
		# Precomputed embedding table [0..18] (0 padding)
	...
	def _extract_features_from_indexed(self, hand_idx: np.ndarray, disc_idx: np.ndarray, called_idx: np.ndarray, game_state_vec: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
		...
		hand_emb = self._embedding_table[np.clip(hand_idx_safe, 0, 18)]
		hand_seq = np.transpose(hand_emb, (1, 0))  # (embed, 12)
		# Called per player -> flatten top 9
		called = np.asarray(called_idx, dtype=np.int32)
		if called.ndim != 3:
			called = np.zeros((4, 3, 3), dtype=np.int32)
		...
		called_top9 = called_reordered[:, : self._max_called_tiles_per_player]
		calls_emb = self._embedding_table[np.clip(called_top9, 0, 18)]  # (4,9,embed)
		calls_seq = np.transpose(calls_emb.reshape(-1, calls_emb.shape[-1]), (1, 0))  # (embed,36)
		...
		disc_emb = self._embedding_table[np.clip(disc_slice, 0, 18)]  # (4,K,embed)
	...
	def evaluate(self, game_state: GamePerspective) -> Tuple[Dict[str, np.ndarray], float]:
		...
		called_idx = idx.get('called_sets_idx', np.zeros((4, 3, 3), dtype=np.int32))
